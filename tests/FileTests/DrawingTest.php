...
class DrawingTest extends TestCase
{
    ...
    public function testOnePageSmiley(){
        if(!$this->_config['test_open_office']){
            $this->assertTrue(true);
            return;
        }
        $session = $this->_session;
        $file = $session->newFile(__DIR__ . "/../TestFiles/drawings/1pagesmiley.odg");
        $file->create();
    ...
    public function testTwoPageSmiley(){
        if(!$this->_config['test_open_office']){
            $this->assertTrue(true);
            return;
        }
        $session = $this->_session;
        $file = $session->newFile(__DIR__ . "/../TestFiles/drawings/2pages.odg");
        $file->create();
        $this->assertEquals(0, count($file->getWarnings()));
        ...
        //use PdfInfo to check if we indeed have no error in the output document.
        $info = new PdfInfo($nam);
        $this->assertEquals(2, $info->pageCount());
        $this->assertEquals(0, count($info->getPDFenErrors()));
        unlink($nam);
        $session->delete();
    }
