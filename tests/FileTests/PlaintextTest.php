...
class PlaintextTest extends TestCase
{
    ...
    public function testOnePage() {
        ...
        $file = $session->newFile(__DIR__ . "/../TestFiles/plaintext/1page1line.rtf");
        $file->create();
    ...
    public function testNinetyPages() {
        $session = $this->_session;
        foreach(['rtf', 'txt'] as $extension) {
            $file = $session->newFile(__DIR__ . "/../TestFiles/plaintext/90pages.$extension");
            $file->create();
            $this->assertEquals(0, count($file->getWarnings()));
            ...
            //use PdfInfo to check if we indeed have no error in the output document.
            $info = new PdfInfo($nam);
            $this->assertGreaterThan(40, $info->pageCount());
            $this->assertLessThan(180, $info->pageCount());
            unlink($nam);
            $file->delete();
        }
    ...
    public function testAccessLog() {
        $session = $this->_session;
        $file = $session->newFile(__DIR__ . "/../TestFiles/plaintext/access_log.log");
        $file->create();
        $this->assertEquals(0, count($file->getWarnings()));
    ...
    public function testSequel() {
        $session = $this->_session;
        $file = $session->newFile(__DIR__ . "/../TestFiles/plaintext/world.sql");
        $file->create();
        $this->assertEquals(0, count($file->getWarnings()));
