...
class PresentationTest extends TestCase
{
    ...
    public function testFiveSlides() {
        $session = $this->_session;
        foreach(['odp', 'pps', 'ppsx', 'ppt', 'pptx'] as $extension) {
            if(!$this->_config['test_open_office'] && $extension[0] === 'o' && $extension[1] === 'd'){
                continue;
            }
            if(!$this->_config['test_pps'] && ($extension === 'pps' || $extension === 'ppsx')) {
                continue;
            }
            $file = $session->newFile(__DIR__ . "/../TestFiles/presentations/5slideswithgraphandimage.$extension");
            $file->create();
